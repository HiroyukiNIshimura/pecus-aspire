...
public class WorkspaceService
{
    ...
    /// <summary>
    /// ワークスペース詳細情報を取得（DTO形式で、IdentityIconUrl を含む）
    /// 管理操作用：有効・無効に関わらずワークスペースを取得します。
    /// </summary>
    /// <param name="workspaceId">ワークスペースID</param>
    public async Task<WorkspaceDetailFullResponse> GetWorkspaceDetailIncludingInactiveAsync(int workspaceId)
    {
        // ワークスペース基本情報を取得（IsActiveフィルタなし）
        var workspace = await _context.Workspaces
            .AsNoTracking()
            .Include(w => w.Genre)
            .Include(w => w.WorkspaceUsers)
