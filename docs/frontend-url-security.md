...
- **コンテキスト**: メール送信時などに使用するフロントエンド URL の生成と検証。
- **重要ルール**:
  - **URL生成**: リクエストボディの `FrontendUrl` をそのまま使用してはならない。
  - **検証**: `Origin` ヘッダーまたは `Referer` ヘッダーを取得し、ホワイトリスト（`AllowedOrigins`）と照合する。
  - **ホワイトリスト**: `appsettings.json` の `Security:AllowedOrigins` で管理。
  - **フォールバック**: 検証失敗時やヘッダー欠落時は、設定されたデフォルト URL を使用する。
- **禁止事項**: `Request.Host` やリクエストパラメータの URL を無検証で使用すること（Host ヘッダーインジェクション/オープンリダイレクト脆弱性）。
