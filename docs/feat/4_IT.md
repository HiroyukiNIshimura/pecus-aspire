...
8. **STフェーズへ引き継ぎ**
  ...
  - ST の具体的な進め方、役割分担、制約は `6_ST.md` を参照する。

> **ITの Pass は UI 品質の最終保証を意味しない。画面品質に関する最終判断は、後続の UI テストフェーズで自動証跡と人間の目視確認を組み合わせて実施する。**
...
- ITフェーズで引き渡すべき主要情報は、**Pass/Fail結果だけでなく、UIテストで再利用できる再現条件とログ証跡**を含む
- 画面に現れる不具合のうち、文言の揺らぎ、見た目の違和感、レスポンシブ時の文字崩れなどは、後続の UI テストフェーズで**自動証跡 + 人間の目視確認**により最終判断する
- したがって、ITフェーズでは「どの条件で画面不整合が起こり得るか」を後続フェーズへ渡せる粒度で整理しておく

### STフェーズへの受け渡し補足

- ITフェーズから ST フェーズへは、**未解決の統合リスク、再現条件、ログ証跡、環境差分情報**を引き渡す
- 特に、本番同等環境でのみ顕在化しうる懸念（スケーリング、外部依存、長時間稼働、監視観点）は、ST 側で再評価しやすい粒度で明記する
- ST の詳細な役割分担と制約は `6_ST.md` を参照する
...
あわせて、後続の UI テストフェーズでは、IT で確認した統合動作を土台にしつつ、**自動証跡では拾い切れない文言品質や見た目品質を人間の目視で補完する**前提で運用する。
