...
・ワークスペースA
  進行中: 12件 / 完了: 5件 / 期限切れ: 2件 ⚠️

・ワークスペースB
  進行中: 8件 / 完了: 3件 / 期限切れ: 0件
...
- 固定: 午前9時（組織のタイムゾーン、または JST 固定）
- ※ 将来的に組織設定で変更可能にすることも検討

### 5.3 集計対象期間

- 配信日の前週（月曜〜日曜）を1週間として集計
...
| 機能 | 説明 |
|------|------|
| 配信時刻のカスタマイズ | 組織設定で配信時刻を変更可能に |
| ユーザー単位のオプトアウト | ユーザー設定でレポート受信を無効化 |
| 前週比較 | 「前週比 +N件」のような比較表示 |
| 平均タスク完了時間 | 作成→完了までの平均日数 |
| HTML リッチメール | グラフや進捗バーを含むリッチな表示 |

---

## 8. 検討事項・未決定事項

- [ ] 配信時刻を固定(9:00)にするか、組織設定で変更可能にするか
- [ ] ユーザー単位でのオプトアウト機能は必要か
- [ ] メールが届かない環境のユーザーへの代替手段（アプリ内通知等）
- [ ] 「今週」の定義（月曜起点 or 日曜起点）を組織設定で変更可能にするか
