...
| **特定回の参加状況** | 「来週だけ不参加」など、特定回のみの参加状況を設定する機能。`AgendaAttendanceResponse`テーブルで実装済み | ✅ 完了 |

※1 この回のみの場合、編集画面で繰り返し設定・参加者設定セクションは非表示になる（参加者変更は非対応）。
...
| **AttendanceDeclined（不参加）** | 参加者 - 変更者 | 不参加時のみ通知（参加・仮参加は通知しない） |
