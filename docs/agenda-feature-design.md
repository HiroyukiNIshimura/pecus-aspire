...
| 項目 | 説明 | 状態 |
|------|------|------|
| **通知一覧UI** | 一覧ページに通知セクションを追加。招待・変更・中止・リマインダー通知を表示。個別/一括既読機能。詳細は2.6節参照 | 🔲 |
...
- `fetchNotifications` - 通知一覧取得（一覧ページで使用予定）
- `markNotificationAsRead` - 個別既読（一覧ページで使用予定）
- `markAllNotificationsAsRead` - 一括既読（一覧ページで使用予定）
...
**実装方針**:
- アジェンダアイコン（📅）をクリックすると一覧ページ（`/agendas`）に遷移
- バッジには未読通知数 + 未回答招待数の合計を表示
- ドロップダウンは実装せず、シンプルなページ遷移方式を採用

### 2.6 通知一覧UI（一覧ページ内）

一覧ページ (`/agendas`) の上部に通知セクションを配置し、未読通知を表示する。
...
│  ── 🔔 通知 (3件) ──────────────────── [すべて既読にする]   │
│  │                                                          │
│  ├─ 🆕 山田太郎さんが「週次定例MTG」に招待しました          │
│  │      1/27(月) 14:00  [参加] [仮] [不参加]                │
│  │                                                          │
│  ├─ 🚫 「プロジェクトレビュー」が中止されました             │
│  │      主催者の都合により中止                              │
│  │                                                          │
...
| 機能 | 詳細 |
|------|------|
| **通知一覧表示** | 未読通知を上部セクションに表示。最大5件、それ以上は「すべて表示」リンク |
| **個別既読** | 通知クリックで詳細ページへ遷移 + 既読API呼び出し |
| **一括既読** | 「すべて既読にする」ボタンで全未読を既読に |
| **招待への返答** | 招待通知から直接参加状況を選択可能（参加/仮/不参加） |
| **通知非表示** | 未読が0件の場合、通知セクション自体を非表示 |
...
Google Calendar / Slack 風のシンプルな検索ベースUI。
...
選択済み:
┌──────────────────────────────────────────────────────────┐
│ [👑 自分(主催者)]  [山田太郎 ×] [開発チーム(15人) ×]     │
└──────────────────────────────────────────────────────────┘
  ↑ 主催者は削除不可
```

**注意**: 主催者（作成者）はバックエンドで自動的に「参加」ステータスで追加されます。
...
| **統合検索** | 1つの検索フィールドでユーザー名・メール・ワークスペース名を検索 |
| **候補表示** | ユーザー → ワークスペース → 組織全体 の順で表示 |
| **クリックで追加** | 候補をクリックすると即座に参加者として追加（WSや組織全体はメンバー全員を追加） |
| **チップ表示** | 選択済みはチップ形式で表示、×で削除可能（主催者は削除不可） |
| **重複排除** | WS追加時、既存ユーザーと重複する場合は自動マージ |
| **自分を除外** | 検索候補に自分自身は表示しない（既に主催者として追加済みのため） |

### 9.4 検索候補の表示ルール

1. **空欄時**: 最近使ったユーザー/WS、または「組織全体」のみ表示
2. **入力時**: 前方一致でユーザー・WS・「組織全体」をフィルタ
3. **上限**: 各カテゴリ最大5件、スクロールで追加読み込み
