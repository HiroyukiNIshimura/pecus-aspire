...
- **コンテキスト**: 大量データ（500M+）のシード投入最適化。
- **重要ルール**:
  - **手法**:
    1. `DISABLE TRIGGER ALL` で制約・トリガーを一時無効化。
    2. `COPY` コマンドまたは生 SQL バルクインサートを使用（EF Core の `AddRange` は遅いため避ける）。
    3. 最後に `ENABLE TRIGGER ALL` と `VACUUM ANALYZE` を実行。
  - **バッチサイズ**: 適切なサイズ（例: 10,000件）で分割コミットする。
- **関連ファイル**: `pecus.Libs/DB/Seed/DatabaseSeeder.cs`
