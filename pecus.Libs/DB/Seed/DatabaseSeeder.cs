...
#pragma warning restore EF1002
    ...
    public async Task SeedUsersAsync()
    {
        ...
        if (!await _context.Users.AnyAsync(u => u.Email == "admin@sample.com"))
        {
            ...
            _context.Users.Add(adminUser);
            _context.Users.Add(ordinaryUser);
            await _context.SaveChangesAsync();
    ...
    {
        // ジャンルごとの説明文テンプレート
        var descriptionsByGenre = new Dictionary<string, string[]>
        {
            ["プロジェクト・開発"] = new[]
            {
        ...
        // ジャンルに対応する説明文配列を取得、なければデフォルト
        if (descriptionsByGenre.TryGetValue(genreName, out var descriptions))
        {
            return descriptions[_random.Next(descriptions.Length)];
        }

        // デフォルトの説明文（ジャンルが見つからない場合）
