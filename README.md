...
> ⚠️ **初回起動時の注意:** 初回起動時はマイグレーションとシードデータの投入（ユーザー200件、ワークスペース100件、関連するアクティビティ等）を行うため、**数分程度の時間がかかります**。Aspire ダッシュボードで `pecus.DbManager` のステータスが Healthy になってもシードデータ投入に数分かかります。
