...
このREADMEもGithub Copilot エージェントが生成。
> 引用部分だけ自分がちょっと手を入れてるだけで、あとは完全にAIが生成している。
> Botのデザイン、テーマ曲もみーーんなAIが生成しています。
...
2026/01/31
> あとはちょこちょこ機能追加などを行っています。

## 📋 プロジェクト概要

**Coati** は、.NET Aspire 13.4.6 を使用した分散マイクロサービス構成のマルチテナント型プロジェクト・タスク管理システムです。
AI支援による高度なタスク管理、リアルタイムコラボレーション、ゲーミフィケーション要素を備えた次世代のチーム管理プラットフォームです。
...
> ‼️ 注意
> デモサイトではお金ないのでDeepSeekを利用しています。
> 好きに試してもらって構いませんが、個人や組織の重要なデータや秘匿情報は入力しないでください。
> 入力データに対する保証は当方では一切の責任を負いません。
...
- **pecus.AppHost** - Aspire オーケストレーションホスト（サービス起動・構成管理）。開発環境用です。
...
- @iconify/tailwind4 1.2.3 (アイコン)
- Biome 2.5.0 (リンター/フォーマッター)
...
## 🎧 テーマソング
https://on.soundcloud.com/OKCH3aciqJcu2EkKUg
> ログインしなくても、Signinのダイアログをクローズすれば視聴可能です。
> 音が出るので注意してください。
