...
**Coati** は、.NET Aspire 10.0 を使用した分散マイクロサービス構成のマルチテナント型プロジェクト・タスク管理システムです。
...
- **高度なタスク管理** - 優先度、進捗率、見積もり・実績時間、先行タスク依存関係
- **タスクフロー図** - 視覚的なタスク依存関係の表示
- **担当者負荷チェック** - メンバーの期限日別タスク負荷を確認
- **タグ・関連付け** - アイテムのタグ管理と相互関連付け

### 🤖 AI支援機能
- **AIタスク生成** - タスク候補の自動生成、クリティカルパス分析、見積もり提案
