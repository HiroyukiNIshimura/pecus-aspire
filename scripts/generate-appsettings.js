...
function generateFrontendEnv(infra, env) {
  ...
  const lines = [
    ...
    '# ============================================',
    '#',
    '# 【用途】開発環境 (Aspire なし) のフォールバック用',
    '#',
    '# - Aspire 環境では、これらの値は自動的に注入されるため本ファイルは不要',
    '# - Next.js 単体で `npm run dev` する場合にのみ使用される',
    '# - 本番ビルドでは .dockerignore で除外されており、--build-arg で渡す',
    '#',
    '',
  ];
  ...
  if (isDev) {
    lines.push(
      '# 開発環境: 自己署名証明書を許可（Aspire は HTTPS + 自己署名証明書を使用）',
  ...
  lines.push(
    '# アプリドメイン（サーバーサイド、next.config.ts で使用）',
    `NEXT_ALLOW_DOMAIN=${infra.urls.allowDomain || 'localhost'}`,
    '',
    '# API URL（サーバーサイド、src/libs/env.ts で使用）',
    `PECUS_API_URL=${infra.urls.webapiPublic}`,
    '',
    '# API URL（クライアントサイド、ビルド時に埋め込まれる）',
    `NEXT_PUBLIC_API_URL=${infra.urls.webapiPublic}`,
    '',
  );
