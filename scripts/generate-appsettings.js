...
function generate() {
  ...
  // Prometheus 設定ファイル（monitoring.enabled が true の場合のみ）
  generatePrometheusConfig(_infrastructure, env);
  ...
  const prometheusCount = _infrastructure.monitoring?.enabled ? (env === 'prod' ? 2 : 1) : 0;
  const totalCount = baseCount + dockerEnvCount + prometheusCount;
...
/**
 * Prometheus 設定ファイルを生成
 * @param {object} infra - インフラ設定
 * @param {string|null} env - 環境 ('dev' | 'prod' | null)
 */
function generatePrometheusConfig(infra, env) {
  const monitoring = infra.monitoring;

  if (!monitoring || !monitoring.enabled) {
    console.log('Skipped: Prometheus config (monitoring.enabled is false)');
    return;
  }

  const prometheusDir = path.join(ROOT_DIR, 'deploy-bluegreen', 'ops', 'prometheus');
  if (!fs.existsSync(prometheusDir)) {
    fs.mkdirSync(prometheusDir, { recursive: true });
    console.log('Created directory:', prometheusDir);
  }

  const docker = infra.docker || {};
  const ports = infra.ports || {};

  const webapiHost = docker.webapiHost || 'pecusapi';
  const frontendHost = docker.frontendHost || 'frontend';
  const lexicalHost = docker.lexicalConverterHost || 'lexicalconverter';

  const webapiPort = ports.webapi || 7265;
  const frontendPort = ports.frontend || 3000;
  const lexicalMetricsPort = ports.lexicalConverterMetrics || 9101;

  const nodeExporterPort = monitoring.nodeExporter?.port || 9100;
  const blackboxPort = monitoring.blackboxExporter?.port || 9115;

  const isProd = env === 'prod';

  if (isProd) {
    const prodConfig = generatePrometheusYaml({
      webapiTargets: [`${webapiHost}-blue:${webapiPort}`, `${webapiHost}-green:${webapiPort}`],
      frontendTargets: [`${frontendHost}-blue:${frontendPort}`, `${frontendHost}-green:${frontendPort}`],
      lexicalTargets: [`${lexicalHost}-blue:${lexicalMetricsPort}`, `${lexicalHost}-green:${lexicalMetricsPort}`],
      nodeExporterTarget: `node-exporter:${nodeExporterPort}`,
      blackboxExporterTarget: `blackbox-exporter:${blackboxPort}`,
      healthTargets: [
        `http://${webapiHost}-blue:${webapiPort}/health`,
        `http://${webapiHost}-green:${webapiPort}/health`,
        `http://${frontendHost}-blue:${frontendPort}/health`,
        `http://${frontendHost}-green:${frontendPort}/health`,
      ],
    });

    const prodPath = path.join(prometheusDir, 'prometheus.yml');
    fs.writeFileSync(prodPath, prodConfig);
    console.log('Generated:', prodPath);
  }

  const devConfig = generatePrometheusYaml({
    webapiTargets: [`host.docker.internal:${webapiPort}`],
    frontendTargets: [`host.docker.internal:${frontendPort}`],
    lexicalTargets: [`host.docker.internal:${lexicalMetricsPort}`],
    nodeExporterTarget: `host.docker.internal:${nodeExporterPort}`,
    blackboxExporterTarget: `host.docker.internal:${blackboxPort}`,
    healthTargets: [],
    isDev: true,
    useHttps: true,
  });

  const devPath = path.join(prometheusDir, 'prometheus.dev.yml');
  fs.writeFileSync(devPath, devConfig);
  console.log('Generated:', devPath);
}

/**
 * Prometheus YAML 設定を生成
 */
function generatePrometheusYaml(opts) {
  const {
    webapiTargets,
    frontendTargets,
    lexicalTargets,
    nodeExporterTarget,
    blackboxExporterTarget,
    healthTargets,
    isDev = false,
    useHttps = false,
  } = opts;

  const formatTargets = (targets) => targets.map((t) => `        - '${t}'`).join('\n');
  ...
  const lines = [
    ...
    '# Do NOT edit this file directly!',
    '# ============================================',
    isDev ? '# Development environment (Aspire)' : '# Production environment (Blue/Green)',
    '',
    'global:',
    '  scrape_interval: 15s',
  ...
  // Backend job
  if (webapiTargets.length > 0) {
    lines.push('  - job_name: "backend"', '    metrics_path: /metrics');
    if (useHttps) {
      lines.push('    scheme: https', '    tls_config:', '      insecure_skip_verify: true');
    }
    lines.push('    static_configs:', '      - targets:', formatTargets(webapiTargets), '');
  }
  ...
  if (frontendTargets.length > 0) {
    lines.push(
      ...
      '    static_configs:',
      '      - targets:',
      formatTargets(frontendTargets),
      '',
    );
  }

  // LexicalConverter job (本番のみ、または実装済みの場合)
  if (lexicalTargets.length > 0) {
    lines.push(
      '  - job_name: "lexicalconverter"',
      '    metrics_path: /metrics',
      '    static_configs:',
      '      - targets:',
      formatTargets(lexicalTargets),
      '',
    );
  }

  // Node exporter job (本番のみ)
  if (!isDev) {
    lines.push('  - job_name: "node"', '    static_configs:', `      - targets: ['${nodeExporterTarget}']`, '');
  }

  // Blackbox job (外形監視、本番のみ)
  if (healthTargets.length > 0) {
    lines.push(
      ...
      `        replacement: '${blackboxExporterTarget}'`,
      '',
    );
  }
