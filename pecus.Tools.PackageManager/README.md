# Pecus.Tools.PackageManager

NuGet パッケージの依存関係を管理し、更新可能なパッケージを可視化するツールです。

## 概要

このツールは `dotnet-outdated-tool` をラップし、以下の機能を提供します：

- ✅ ソリューション全体のパッケージバージョンチェック
- 📊 依存関係グラフの構築
- 🎯 安全にアップデート可能なパッケージの特定
- 📈 更新重要度（Patch/Minor/Major）の判定
- 🔍 影響を受けるプロジェクトの可視化

## 前提条件

### dotnet-outdated-tool のインストール

```bash
dotnet tool install --global dotnet-outdated-tool
```

インストール確認：

```bash
dotnet outdated --version
```

## インストール

### ローカルビルド

```bash
# プロジェクトディレクトリでビルド
dotnet build pecus.Tools.PackageManager/pecus.Tools.PackageManager.csproj

# 実行
dotnet run --project pecus.Tools.PackageManager -- check
```

### グローバルツールとしてインストール（任意）

```bash
# パッケージング
dotnet pack pecus.Tools.PackageManager/pecus.Tools.PackageManager.csproj

# グローバルインストール
dotnet tool install --global --add-source ./pecus.Tools.PackageManager/nupkg Pecus.Tools.PackageManager

# 実行
pecus-pkg check
```

## 使用方法

### 基本コマンド

#### パッケージチェック

```bash
# デフォルト（Patch レベルの更新のみ表示）
dotnet run --project pecus.Tools.PackageManager -- check

# すべての更新可能なパッケージを表示
dotnet run --project pecus.Tools.PackageManager -- check --all

# 特定のソリューションファイルを指定
dotnet run --project pecus.Tools.PackageManager -- check --solution path/to/solution.sln
```

#### オプション

| オプション | 説明 | 例 |
|---|---|---|
| `--solution` | ソリューションファイルのパスを指定します。省略時は `pecus.sln` を使用します。 | `--solution path/to/solution.sln` |
| `--all` | Patch 以外（Minor/Major）も含めて全て表示します。 | `--all` |

#### パッケージ更新

```bash
# 自動更新（既定: Patch のみ）
dotnet run --project pecus.Tools.PackageManager -- update

# Minor/Major も含めて更新
dotnet run --project pecus.Tools.PackageManager -- update --all

# プロンプト更新（対話式）
dotnet run --project pecus.Tools.PackageManager -- update --mode prompt

# 特定のソリューションファイルを指定
dotnet run --project pecus.Tools.PackageManager -- update --solution path/to/solution.sln
```

#### オプション

| オプション | 説明 | 例 |
|---|---|---|
| `--solution` | ソリューションファイルのパスを指定します。省略時は `pecus.sln` を使用します。 | `--solution path/to/solution.sln` |
| `--mode` | 更新モード。`auto`（自動）または `prompt`（対話式）を指定します。 | `--mode prompt` |
| `--include-auto-references` | 自動参照パッケージを含めます（既定: true）。 | `--include-auto-references false` |
| `--all` | Minor/Major も含めて更新します（既定は Patch のみ）。 | `--all` |

### 出力例

```
✓ パッケージをスキャン中...
┌──────────────────────────────────────┬─────────┬─────────┬────────┬────────────────────────┐
│            パッケージ                 │  現在   │  最新   │ 重要度 │    影響プロジェクト     │
├──────────────────────────────────────┼─────────┼─────────┼────────┼────────────────────────┤
│ Microsoft.EntityFrameworkCore        │ 10.0.0  │ 10.0.1  │ Patch  │ pecus.Libs, pecus.WebApi│
│ Spectre.Console                      │ 0.49.0  │ 0.49.1  │ Patch  │ pecus.Tools.PackageManager│
└──────────────────────────────────────┴─────────┴─────────┴────────┴────────────────────────┘

合計: 2 個のパッケージが更新可能
  Patch: 2  Minor: 0  Major: 0
```

## 更新重要度の判定基準

| 重要度 | 説明 | 表示色 | 推奨 |
|-------|------|-------|------|
| **Patch** | バグ修正のみ（x.x.X） | 🟢 Green | 自動更新推奨 |
| **Minor** | 後方互換性あり機能追加（x.X.x） | 🟡 Yellow | テスト後更新 |
| **Major** | 破壊的変更（X.x.x） | 🔴 Red | 慎重に検討 |

## 開発ガイドライン

### プロジェクト構造

```
pecus.Tools.PackageManager/
├── Commands/
│   └── CheckCommand.cs       # CLI コマンド実装
├── Services/
│   ├── DotnetOutdatedService.cs  # dotnet-outdated ラッパー
│   └── PackageAnalyzer.cs        # 依存関係分析
├── Models/
│   └── PackageModels.cs      # データモデル
├── Program.cs                # エントリポイント
└── pecus.Tools.PackageManager.csproj
```

### 今後の拡張予定

- [ ] `analyze` コマンド: 依存関係グラフの視覚化
- [ ] `report` コマンド: Markdown/HTML レポート生成
- [ ] CI/CD 統合: GitHub Actions でスケジュール実行

## トラブルシューティング

### エラー: dotnet-outdated tool is not installed

```bash
dotnet tool install --global dotnet-outdated-tool
```

### エラー: ソリューションファイルが見つかりません

ソリューションファイルのパスを明示的に指定してください：

```bash
dotnet run --project pecus.Tools.PackageManager -- check --solution /path/to/pecus.sln
```

## ライセンス

Pecus Aspire プロジェクトと同じライセンスに従います。
