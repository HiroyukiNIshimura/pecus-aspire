 ...
 * - 繰り返しイベントの特定回のみを中止する
 * - 中止理由の入力（任意）
 * - cancelOccurrence Server Actionを呼び出す
 */
            ...
            <div className="alert alert-warning">
              ...
              <div>
                <p className="font-medium">
                  「{agendaTitle}」の {occurrenceDate} の回を中止しますか？
                </p>
                <p className="text-sm mt-1">この回のみ中止されます。他の回には影響しません。</p>
              </div>
            </div>

            {/* 中止理由入力 */}
            <div className="form-control">
              <label htmlFor="occurrence-cancellation-reason" className="label">
                <span className="label-text font-semibold">中止理由（任意）</span>
              </label>
              <textarea
                ref={reasonInputRef}
                id="occurrence-cancellation-reason"
                className="textarea textarea-bordered w-full"
                placeholder="例: 会議室が使用できないため、この回は中止します。"
                rows={3}
                maxLength={500}
                value={reason}
