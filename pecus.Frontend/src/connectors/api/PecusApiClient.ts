...
function extractFilenameFromContentDisposition(contentDisposition: string | undefined): string | null {
  ...
  // filename*=UTF-8''encoded-filename 形式（RFC 5987）
  const filenameStarMatch = contentDisposition.match(/filename\*=(?:UTF-8''|utf-8'')([^;]+)/i);
  if (filenameStarMatch) {
    return decodeURIComponent(filenameStarMatch[1]);
  }

  // filename="filename" または filename=filename 形式
...
/**
 * ファイルダウンロード用の関数
 * Content-Disposition ヘッダーからファイル名を取得可能
 *
 * @example
 * const result = await downloadFile('/api/files/download', { fileId: '123' });
 * if (result.filename) {
 *   // サーバー指定のファイル名を使用
