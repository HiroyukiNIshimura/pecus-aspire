...
interface EditWorkspaceTaskModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  workspaceId: number;
...
export default function EditWorkspaceTaskModal({
  ...
  const fetchTask = useCallback(
    ...
    [workspaceId, itemId, syncTaskToForm, notify],
  );

  // 初期化
  useEffect(() => {
    if (isOpen && initialNavigation) {
  ...
  }, [isOpen, initialNavigation, syncTaskToForm]);
  ...
  const handleNextTask = useCallback(async () => {
    ...
    if (currentPage < totalPages) {
      ...
      try {
        ...
        notify.error('次のタスクの取得に失敗しました');
      } finally {
        setIsLoadingTask(false);
      }
    }
  }, [navigation, isLoadingTask, workspaceId, itemId, pageSize, fetchTask, notify]);
      ...
      onSubmit: async (data) => {
        ...
        if (!result.success) {
          if (result.error === 'conflict') {
            ...
            notify.error('他のユーザーが更新しました。最新のデータを取得します。');
