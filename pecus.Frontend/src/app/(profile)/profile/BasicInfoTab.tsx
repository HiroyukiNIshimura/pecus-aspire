...
export default function BasicInfoTab({ user, onUpdate, notify, isLoading, setIsLoading }: BasicInfoTabProps) {
  // フォームバリデーションスキーマの定義
  // displayName はブラウザのサインイン自動入力を回避するための名称（内部的には username）
  const basicInfoSchema = z.object({
    displayName: usernameSchema,
  });

  const [selectedAvatarType, setSelectedAvatarType] = useState<'Gravatar' | 'UserAvatar' | 'AutoGenerated'>(
    ...
    onSubmit: async (data) => {
      try {
        const result = await updateProfile({
          username: data.displayName !== user.username ? data.displayName : undefined,
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      notify.error(error instanceof Error ? error.message : 'アップロードに失敗しました');
      setAvatarPreviewUrl(null);
    } finally {
      setIsLoading(false);
    }
    ...
    if (uploadedFileUrl && user.id) {
      ...
      if (fileName) {
        ...
        try {
          ...
          notify.error(error instanceof Error ? error.message : '削除に失敗しました');
        } finally {
          setIsLoading(false);
        }
      }
    ...
    <form ref={formRef} onSubmit={handleSubmit} noValidate className="space-y-6 bg-base-100">
      <div className="form-control">
        ...
        <input
          ...
          className={`input input-bordered ${shouldShowError('displayName') ? 'input-error' : ''}`}
        ...
        <div className="form-control">
          ...
          >
            ...
            {avatarPreviewUrl ? (
                ...
                <div className="space-y-2">
                  {uploadedFileUrl ? (
                    <div className="flex items-center gap-2 justify-center">
                      ...
                      <p className="text-sm font-medium text-base-content">新しい画像をアップロード完了</p>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 justify-center">
                      <span className="loading loading-spinner loading-sm"></span>
                      <p className="text-sm font-medium text-base-content">アップロード中...</p>
                    </div>
                  )}
                  <button
                    type="button"
              ...
              <div className="flex flex-col items-center gap-4 cursor-pointer" onClick={handleClickUpload}>
