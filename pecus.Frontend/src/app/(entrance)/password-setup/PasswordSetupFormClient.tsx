...
import { setPasswordSchema } from '@/schemas/signInSchemas';

/**
 * パスワード設定フォーム (Client Component)
 *
 * 責務:
 * - URLからトークンを取得
 * - パスワード入力・確認入力の受付
 * - バリデーション（Zod）
 * - Server Action でパスワード設定 API を呼び出し
 */
export default function PasswordSetupFormClient() {
    ...
    return (
      <div className="card w-full max-w-sm shadow-lg bg-base-100">
        <div className="card-body">
          <h2 className="card-title text-center w-full mb-6">パスワード設定</h2>
          <div className="alert alert-error">
            <span className="icon-[tabler--alert-circle] size-5 shrink-0" />
            <span>パスワード設定用のトークンが見つかりません。メールに記載されたリンクからアクセスしてください。</span>
          </div>
  ...
  if (successMessage) {
    return (
      <div className="card w-full max-w-sm shadow-lg bg-base-100">
        <div className="card-body">
          <h2 className="card-title text-center w-full mb-6">パスワード設定完了</h2>
    ...
    <div className="card w-full max-w-sm shadow-lg bg-base-100">
      <div className="card-body">
        ...
        <p className="text-sm text-base-content/70 mb-4">
          アカウントのパスワードを設定してください。パスワードは8文字以上で、大文字・小文字・数字を含む必要があります。
        </p>
