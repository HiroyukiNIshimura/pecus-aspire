...
function codeNodeTransform(node: CodeNode, editor: LexicalEditor, tokenizer: Tokenizer) {
  ...
  // dynamic import of dual themes (light/dark)
  let inFlight = false;
  if (!areDualThemesLoaded()) {
    loadDualThemes(editor, nodeKey);
    inFlight = true;
  }
 ...
 */
function $getCodeLines(selection: RangeSelection): Array<Array<CodeHighlightNode | TabNode>> {
...
function $handleTab(shiftKey: boolean): null | LexicalCommand<void> {
  ...
  let selectionFirst: PointType;
  let selectionLast: PointType;
...
function $handleMultilineIndent(type: LexicalCommand<void>): boolean {
  ...
  for (let i = 0; i < codeLinesLength; i++) {
    ...
    if (line.length > 0) {
      let firstOfLine: null | CodeHighlightNode | TabNode | LineBreakNode = line[0];
...
function $handleShiftLines(type: LexicalCommand<KeyboardEvent>, event: KeyboardEvent): boolean {
  ...
  let start: CodeHighlightNode | TabNode | LineBreakNode;
  let end: CodeHighlightNode | TabNode | LineBreakNode;
  ...
  linebreak.remove();
  // biome-ignore lint/suspicious/useIterableCallbackReturn: intentionally using forEach for side effects
...
export function registerCodeHighlighting(editor: LexicalEditor, tokenizer?: Tokenizer): () => void {
  ...
  registrations.push(
    editor.registerNodeTransform(CodeNode, (node) => codeNodeTransform(node, editor, tokenizer)),
    editor.registerNodeTransform(TextNode, (node) => $textNodeTransform(node, editor, tokenizer)),
    editor.registerNodeTransform(CodeHighlightNode, (node) => $textNodeTransform(node, editor, tokenizer)),
    ...
    editor.registerCommand(
      INDENT_CONTENT_COMMAND,
      (_payload): boolean => $handleMultilineIndent(INDENT_CONTENT_COMMAND),
      COMMAND_PRIORITY_LOW,
    ),
    editor.registerCommand(
      OUTDENT_CONTENT_COMMAND,
      (_payload): boolean => $handleMultilineIndent(OUTDENT_CONTENT_COMMAND),
      COMMAND_PRIORITY_LOW,
    ),
    editor.registerCommand(
      KEY_ARROW_UP_COMMAND,
